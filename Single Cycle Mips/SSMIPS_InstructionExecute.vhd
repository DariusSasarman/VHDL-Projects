...
entity SSMIPS_InstructionExecute is
...
end SSMIPS_InstructionExecute;

architecture Behavioral of SSMIPS_InstructionExecute is
...
ADDRESS_ADDER : process(PC_INC, EXT_IMM)
begin
    BranchAddress <= std_logic_vector(unsigned(PC_inc) + unsigned(Ext_imm));
end process;
...
zero <= '1' when alu_outp_val = x"0000" else '0';
negative <= '1' when (signed(alu_outp_val) < 0) else '0';
